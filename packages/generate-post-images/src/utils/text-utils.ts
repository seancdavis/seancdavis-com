 ...
 */
function fontString(fontSize: number) {
...
function splitLines(title: string): string[] {
  ...
  const firstLine = allWords.reduce((prev, cur) => {
    ...
    if (!lineBroken && withNextWord.length < maxChars) return withNextWord;
...
export function formatTitle(
  title: string,
  context: ReturnType<Canvas["getContext"]>,
  options: TitleFormattingOptions
  ...
  if (width <= options.maxLineWidth) return { fontSize, text };
  // Next, try to reduce the font size with a single line and see if we meet the
  // minimum threshold.
  const singleLineFontSize = findMaxFontSize(
    title,
    context,
    options.maxFontSize,
  ...
  fontSize = Math.min(
    ...text.map((line) =>
      findMaxFontSize(line, context, options.maxFontSize, options.maxLineWidth)
    )
  );
