...
export default function Particles({ count = 10, images }) {
  ...
  useEffect(() => {
    ...
    const reset = () => {
      ...
      //
      // Note: The parent element is actually the grandparent, because slinkity
      // wraps this component in a tag.
      parentElement = canvas.current.parentElement.parentElement;
      ...
      canvas.current.setAttribute("height", canvasHeight);
      canvas.current.setAttribute("width", canvasWidth);
    };
    ...
    const initParticle = () => {
      ...
      return {
        ...
        image: sample(particleImages.current),
    ...
    // Listen for window resizing, and reset accordingly.
    window.addEventListener("resize", () => {
      reset();
      particles = particles.map(initParticle);
    });
  }, []);

  return (
    <>
      <canvas ref={canvas} className="top-0 left-0 absolute" />
      <div className="hidden">
        {images.length > 0 &&
          images.map((name, idx) => (
            <img
              key={idx}
              src={`/images/particles/${name}.svg`}
              ref={(el) => (particleImages.current[idx] = el)}
            />
          ))}
      </div>
    </>
